...
function buildCRMPrompt(context, userMessage) {
    const parts = [];
...
- Health Score: ${context.account.healthScore ?? "Unknown"}`);
    ...
    if (context.deals.length > 0) {
        const dealList = context.deals.map((d) => `- ${d.name}: $${d.amount.toLocaleString()} (${d.stage}, ${d.probability}% probability)`).join("\n");
        parts.push(`\nOpen Deals:\n${dealList}`);
    }
    if (context.tickets.length > 0) {
        const ticketList = context.tickets.map((t) => `- ${t.subject} [${t.status}] Priority: ${t.priority}`).join("\n");
        parts.push(`\nRecent Tickets:\n${ticketList}`);
    }
...
export class CRMAgent {
    ...
    async respond(context, userMessage) {
    ...
    async *respondStream(context, userMessage) {
        const prompt = buildCRMPrompt(context, userMessage);
        try {
