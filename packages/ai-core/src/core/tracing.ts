...
const stepLatencyHistograms: Record<PipelineStep, Histogram> = {} as Record<PipelineStep, Histogram>;
const stepCounter: Record<PipelineStep, Counter> = {} as Record<PipelineStep, Counter>;
...
export function createSpan(name: PipelineStep, traceId?: string): Span {
...
function spanLatency(span: Span, latencyMs: number, error?: unknown): void {
...
export function recordRequest(latencyMs: number, error?: unknown): void {
...
export function getCacheMetrics(): { hits: number; misses: number; ratio: number } {
  const total = totalHits + totalMisses;
  return {
    hits: totalHits,
    misses: totalMisses,
