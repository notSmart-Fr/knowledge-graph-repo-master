...
function createMockIdempotencyStore(calls, isDuplicate = false) {
    return {
        checkAndSet: async (...args) => {
            calls.checkAndSet.push(args);
            return isDuplicate;
        },
    };
}
function createMockCacheStore(calls, cachedResponse) {
    return {
        check: async (...args) => {
...
function createMockContactStore(calls, contact) {
    return {
        getByPhone: async (...args) => {
...
function createMockGraphRetriever(calls, context) {
    return {
        expandFromContact: async (...args) => {
...
function createMockAgentProvider(calls, response) {
    return {
        generate: async (...args) => {
